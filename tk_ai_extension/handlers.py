...
class MCPChatHandler(JupyterHandler):
    ...
    def _build_system_prompt(self, notebooks_dir: Path, notebook_path: str = None) -> str:
        ...
        prompt_parts.extend([
            ...
            "",
            "## CRITICAL: Cell Numbering",
            "- JupyterLab shows execution count [N] in the UI",
            "- ALL cell operations use 0-based index (cell_index), NOT execution count",
            "- ALWAYS call list_cells FIRST before any cell operation (insert, delete, move, overwrite)",
            "- list_cells shows BOTH index and execution count - use the 'index' column for operations",
            "- When user says 'cell 3', confirm: ask if they mean index 3 or execution count [3]",
            "",
